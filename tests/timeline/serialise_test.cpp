...
TEST(Serialise, WritesTracksAndClips) {
    ...
    ASSERT_TRUE(document.add_clip(video, "a.mp4", 100, 0, 9000).has_value());
    ASSERT_TRUE(document.add_clip(video, "b.mp4", 0, 9000, 4500).has_value());
    ...
    EXPECT_EQ(serialise(document),
              ...
              "  clip 2 100 0 9000 1 \"a.mp4\"\n"
              "  clip 3 0 9000 4500 1 \"b.mp4\"\n"
...
TEST(Serialise, IsIndependentOfInsertionOrder) {
    ...
    ASSERT_TRUE(forwards.add_clip(a, "first.mp4", 0, 0, 100).has_value());
    ASSERT_TRUE(forwards.add_clip(a, "second.mp4", 0, 100, 100).has_value());
    ...
    ASSERT_TRUE(backwards.add_clip(b, "second.mp4", 0, 100, 100).has_value());
    ASSERT_TRUE(backwards.add_clip(b, "first.mp4", 0, 0, 100).has_value());
...
TEST(Serialise, DistinguishesDocumentsThatDifferOnlyByAFlag) {
    ...
    const ClipId clip_a = plain.add_clip(a, "x.mp4", 0, 0, 100).value();

    Document flagged = make_document();
    const TrackId b = flagged.add_track(TrackKind::video, "V1").value();
    const ClipId clip_b = flagged.add_clip(b, "x.mp4", 0, 0, 100).value();
